...
export default function InteractivePage() {
  ...
  const handleNext = () => {
    if (contract.step < totalSteps) {
      ...
      // Final step — save contract and redirect
      const finalContract = {
      ...
      localStorage.setItem('iam_interactive_contract', JSON.stringify(finalContract));
          ...
          <div style={{ width: '100%', maxWidth: 800, textAlign: 'center' }}>
            ...
            <div style={{
              ...
              {BUSINESS_TYPES.map(bt => (
                <button
                  key={bt.id}
                  onClick={() => setContract(prev => ({ ...prev, businessType: bt.id }))}
                  style={{
                    padding: '16px 12px',
                    borderRadius: 12,
                    border: contract.businessType === bt.id
                      ? '2px solid #FF6B35'
                      : '1px solid rgba(255,255,255,0.1)',
                    background: contract.businessType === bt.id
          ...
          <div style={{ width: '100%', maxWidth: 600, textAlign: 'center' }}>
            ...
            <div style={{
              ...
              {STYLES.map(s => (
                <button
                  key={s.id}
                  onClick={() => setContract(prev => ({ ...prev, style: s.id }))}
                  style={{
                    padding: '24px 16px',
                    borderRadius: 12,
                    border: contract.style === s.id
                      ? '2px solid #FF6B35'
                      : '1px solid rgba(255,255,255,0.1)',
                    background: contract.style === s.id
                      ? 'rgba(255,107,53,0.12)'
                      : 'rgba(255,255,255,0.03)',
                    color: '#fff',
                    cursor: 'pointer',
                ...
                >
                  <div style={{
                    ...
                    background: s.color,
                    border: s.id === 'light' ? '1px solid rgba(255,255,255,0.2)' : 'none',
                  }} />
                  {s.label}
                </button>
              ))}
