...
export const LayersPanel = () => {
          ...
          >
            ...
            {hasChildren ? (
              <button
                ...
                className="w-4 h-4 flex items-center justify-center text-[9px] text-gray-500 hover:text-gray-300 shrink-0"
              >
                {isCollapsed ? '▸' : '▾'}
              </button>
            ) : (
            ...
            {/* Name */}
            <span className={`text-[11px] font-medium truncate flex-1 ${isRoot ? 'text-gray-500' : ''}`}>
            ...
            {!isRoot && isSelected && (
              <button
                ...
                className="w-5 h-5 flex items-center justify-center rounded text-[10px] text-gray-500 hover:text-red-400 hover:bg-red-900/30 opacity-0 group-hover:opacity-100 transition-opacity shrink-0 mr-1"
                title="Delete"
              >✕</button>
            )}
          </div>
  ...
  return (
    <div className="w-52 bg-[#1e1e1e] border-r border-gray-700/60 flex flex-col shrink-0 overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-700/60 shrink-0">
        <span className="text-sm font-semibold text-white tracking-wide">Layers</span>
      </div>
