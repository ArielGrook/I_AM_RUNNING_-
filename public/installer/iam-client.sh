...
Options:
  ...
  --client-app-pm2-name=NAME   PM2 process name of the customer's existing app
                                 (deploy endpoint will restart this, not our process)
  --mode=MODE                  Installation mode: team (default) or solo
                                 Web-installed instances should always be team.
...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --client-app-pm2-name=*) CLIENT_APP_PM2_NAME="${1#*=}" ;;
      --mode=*) MODE="${1#*=}" ;;
  ...
  for jsonf in tasks.json messages.json conversations.json goals.json \
    ...
    if [ ! -f "$target" ] || [ ! -s "$target" ]; then
      case "$jsonf" in
        ...
        settings.json)      echo "{ \"mode\": \"$MODE\" }" > "$target" ;;
        *)                  echo '{}' > "$target" ;;
      esac
    fi
  done
...
step_secrets() {
  ...
  # Integration mode: if --project-path given, MCP tools see customer's app (PROJECT_ROOT)
  # and deploy endpoint rebuilds/restarts customer's pm2 process (CLIENT_APP_PM2_NAME).
  # Our own system lives separately: IAM_SELF_PATH + IAM_PROCESS_NAME (used by /self-update).
  local effective_project_root="$INSTALL_PATH"
  local effective_client_pm2="$IAM_PROCESS_NAME"
  if [ -n "$PROJECT_PATH" ]; then
    effective_project_root="$PROJECT_PATH"
    if [ -n "$CLIENT_APP_PM2_NAME" ]; then
      effective_client_pm2="$CLIENT_APP_PM2_NAME"
    else
      warn "PROJECT_PATH set but --client-app-pm2-name not provided — deploy endpoint will still restart $IAM_PROCESS_NAME (our own). Pass --client-app-pm2-name=<customer-app> for true Integration mode."
    fi
  fi
...
IAM_SELF_PATH=$INSTALL_PATH
CLIENT_APP_PM2_NAME=$effective_client_pm2
